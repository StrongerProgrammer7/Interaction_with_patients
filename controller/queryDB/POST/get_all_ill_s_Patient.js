...
const select_all_ills_patient = async (req,res) =>
{
    ...
    {
        getIll(`SELECT id FROM Doctor Where account_ethereum = ?`,`SELECT Records.id,Patient.surname as surname,Patient.id as id_patient,Patient.account_ethereum as meta,Diseased.name_ill, Diseased.treatment, Diseased.classification,Records.date_ill,Records.date_cured,Records.status, Patient.list_doctors_have_access as list_doc,Records.id_doctor FROM Records 
        INNER JOIN 
        Doctor ON Doctor.id = Records.id_doctor
        INNER JOIN 
        Diseased ON Diseased.id = Records.id_ill
